...
<div class="shell h-screen w-screen flex flex-col bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100">
  <!-- Main Content Area -->
...
<div class="tab-bar flex items-center h-9 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
...
>
  ...
  {#if tab.connectionColor}
    <span
      ...
      style="background-color: {tab.connectionColor}"
    ></span>
  {/if}
...
<script lang="ts">
  ...
  let queryInfo = $derived(
    activeTab?.type === 'query' ? activeTab.queryResult : null
  );
</script>

<footer class="status-bar flex items-center h-6 px-2 text-xs bg-gray-100 dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700">
  <!-- Connection Status -->
  ...
  return {
    ...
    updateQueryResult(tabId: string, result: QueryResultInfo) {
      update(s => ({
        ...s,
        tabs: s.tabs.map(t =>
          t.id === tabId ? { ...t, queryResult: result } : t
        ),
      }));
    },
