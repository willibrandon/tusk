...
export function createPlanStore() {
  ...
  const allWarnings = $derived.by(() => {
    ...
    return warnings.sort((a, b) => {
      ...
      return severityOrder[a.warning.severity as keyof typeof severityOrder] -
  ...
  return {
    ...
    get expandedNodes() { return state.expandedNodes; },
    get showOnlyWarnings() { return state.showOnlyWarnings; },
    get allWarnings() { return allWarnings; },
...
{#if open}
  ...
  >
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-[420px] max-h-[80vh] overflow-hidden">
      <!-- Header -->
      <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
        <h2 id="explain-dialog-title" class="text-lg font-semibold">
          EXPLAIN Options
        </h2>
      </div>
      ...
      <div class="p-4 space-y-4">
        ...
        {#if options.analyze}
          <div class="p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200
            ...
            <strong>Note:</strong> ANALYZE will actually execute the query.
            For DML statements (INSERT, UPDATE, DELETE), changes will be made
            to the database.
          </div>
        {/if}
      </div>
...
<div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4 space-y-4">
  ...
  <div class="flex items-center justify-between">
    ...
    {#if node.isSlowest}
      <span class="text-xs px-2 py-1 bg-red-100 text-red-700
                   dark:bg-red-900/30 dark:text-red-400 rounded">
        Slowest Node
      </span>
    {/if}
    ...
    <div class="grid grid-cols-2 gap-4 text-sm">
      {#if node.relationName}
        <div>
          ...
          <span class="ml-2 font-mono">{node.schemaName ? `${node.schemaName}.` : ''}{node.relationName}</span>
        </div>
      {/if}
      {#if node.indexName}
        <div>
  ...
  <div class="flex items-center gap-4">
    ...
    <div class="flex-1 h-3 bg-gray-200 dark:bg-gray-700 rounded overflow-hidden">
      <div
        ...
        style="width: {node.percentOfTotal}%"
      ></div>
    </div>
...
<script lang="ts">
  ...
  const maxRow = $derived(
    timelineNodes.reduce((max, n) => Math.max(max, n.row), 0)
  );
  ...
  <div class="flex-1 overflow-auto p-4">
    ...
    {:else if planStore.error}
      <div class="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200
        ...
        <strong>Error:</strong> {planStore.error}
      </div>
...
<ExplainOptionsDialog
  bind:open={showOptionsDialog}
  onrun={handleExplain}
/>
```
...
<pre
  class="font-mono text-sm whitespace-pre overflow-auto p-4
         bg-gray-50 dark:bg-gray-900/50 rounded"
>{@html highlightedText}</pre>
```
