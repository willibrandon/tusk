...
<script lang="ts">
  ...
  const allTables = $derived(
    schemas.flatMap(s => s.tables.map(t => `${s.name}.${t.name}`))
  );
...
{#if open}
  <div
    ...
    aria-modal="true"
  >
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-[600px] max-h-[85vh] flex flex-col">
      <!-- Header -->
      ...
      <div class="flex-1 overflow-auto p-6 space-y-6">
        ...
        <div>
          ...
          <div class="grid grid-cols-2 gap-3">
            {#each [
              { value: 'custom', label: 'Custom (.backup)', desc: 'Recommended. Compressed, selective restore.' },
              { value: 'plain', label: 'Plain SQL (.sql)', desc: 'Human-readable SQL script.' },
              { value: 'directory', label: 'Directory', desc: 'Parallel dump support.' },
              { value: 'tar', label: 'Tar Archive (.tar)', desc: 'Portable archive format.' },
            ] as fmt}
              <label
        ...
        {#if running}
          <div class="space-y-2">
            ...
            {/if}
            {#if output.length > 0}
              <div class="max-h-32 overflow-y-auto bg-gray-100 dark:bg-gray-900 rounded
      ...
      <div class="flex-1 overflow-auto p-6 space-y-6">
        ...
        {#if clean}
          <div class="p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200
            ...
            <strong>Warning:</strong> Clean mode will drop existing objects before restoring.
