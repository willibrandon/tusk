...
export class PostgresCompletionProvider implements monaco.languages.CompletionItemProvider {
	...
	async provideCompletionItems(
		...
		switch (completionContext.type) {
			...
			case 'column':
				suggestions.push(
					...this.getColumnCompletions(completionContext.tables, completionContext.aliases, range)
				);
				break;
	...
	private extractAliases(text: string): Map<string, string> {
		...
		const aliasPattern =
			/\b(\w+(?:\.\w+)?)\s+(?:as\s+)?(\w+)(?=\s*(?:,|\bwhere\b|\bjoin\b|\bon\b|$))/gi;
		let match;
	...
	private extractTablesFromQuery(text: string): string[] {
		...
		const fromMatch = text.match(
			/\bfrom\s+([\w.\s,]+?)(?:\bwhere\b|\bjoin\b|\bgroup\b|\border\b|\blimit\b|$)/i
		);
		if (fromMatch) {
...
<script lang="ts">
	...
	onMount(() => {
		...
		completionProvider = monaco.languages.registerCompletionItemProvider('postgresql', provider);
...
<div class="toolbar">
	<div class="toolbar-group">
		...
		{#if isExecuting}
			<button class="btn btn-danger" onclick={onStop} title="Cancel (Cmd+.)">
				<Square size={16} />
				Stop
			</button>
		{/if}
	...
	<div class="toolbar-group">
		<button class="btn btn-ghost" onclick={onFormat} title="Format SQL (Cmd+Shift+F)">
			<FileText size={16} />
			Format
		</button>

		<button class="btn btn-ghost" onclick={onSave} title="Save (Cmd+S)">
			<Save size={16} />
			Save
		</button>
	</div>
