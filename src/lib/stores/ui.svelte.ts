...
function createUIStore(): UIStoreInterface {
  ...
  const stored = browser ? getStorageItem<UIState>(STORAGE_KEYS.UI_STATE, DEFAULT_STATE) : DEFAULT_STATE;
  ...
  let resultsPanelHeight = $state(clampResultsHeight(stored.resultsPanelHeight ?? DEFAULT_STATE.resultsPanelHeight));
